...
<section class="text-gray-600">
        ...
        <div class="lg:w-2/3 w-full mx-auto overflow-auto">
            <div class="flex items-center justify-between mb-2">
                ...
                <a class="flex ml-auto text-white bg-indigo-500 border-0 py-1.5 px-3 text-sm focus:outline-none hover:bg-indigo-600 rounded"
                    href="{{route('admin.products.create')}}">Adicionar</a>
            </div>
            <table class="table-auto w-full text-left whitespace-no-wrap">
                ...
                <tr @if ($loop->even)class="bg-gray-100"@endif>
                    ...
                    <td class="px-4 py-3 text-sm text-right space-x-3 text-gray-900">
                        <a class="mt-3 text-indigo-500 inline-flex items-center" href="{{route('admin.products.edit', $product->slug)}}">Editar</a>
                        <form action="{{route('admin.products.destroy', $product->slug)}}" method="post">
                            @csrf
                            @method('delete')
                            <button class="mt-3 text-indigo-500 inline-flex items-center" type="submit">Deletar</button>
                        </form>
                    </td>
                </tr>
                @endforeach
    ...
    <div class="flex justify-center items-center list-reset py-2">
